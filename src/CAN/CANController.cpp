...
#include "Logger.h"
#include <ACAN_T4.h> // Ensure the correct library is included
...
void CANController::begin() {
    ...
    if (errorCode == 0) {
        ...
        Logger::log(("Error initializing CAN1: 0x" + String(errorCode, HEX)).c_str());
    }
}
...
void CANController::readEMCY(uint8_t nodeId) {
    ...
    if (ACAN_T4::can1.receive(message) && message.id == (0x080 + nodeId)) { // EMCY response
        ...
        Logger::log(("Received EMCY from Node ID: " + String(nodeId) + ", Error Code: " + String(errorCode, HEX) + ", Error Register: " + String(errorRegister, HEX)).c_str());
    }
}
...
void CANController::writeNMT(uint8_t nodeId, uint8_t command) {
    ...
    ACAN_T4::can1.tryToSend(message);
    Logger::log(("Sent NMT command to Node ID: " + String(nodeId) + ", Command: " + String(command, HEX)).c_str());
}

void CANController::writeHeartbeat(uint8_t nodeId, uint16_t producerTime) {
    writeSDO(nodeId, 0x1017, 0x00, producerTime);
    Logger::log(("Set Heartbeat producer time for Node ID: " + String(nodeId) + " to " + String(producerTime)).c_str());
}

void CANController::readHeartbeat(uint8_t nodeId) {
    int32_t heartbeatTime = readSDO(nodeId, 0x1017, 0x00);
    Logger::log(("Heartbeat producer time for Node ID: " + String(nodeId) + " is " + String(heartbeatTime)).c_str());
}
...
void CANController::logMessage(const char* action, const CANMessage& message) {
    Logger::log((String(action) + " - ID: 0x" + String(message.id, HEX) + ", Length: " + String(message.len) + ", Data: " +
        ...
        String(message.data[6], HEX) + " " + String(message.data[7], HEX)).c_str());
}
